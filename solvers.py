...
def RSDA(X, Y, args, samples, weighted_pseu_label, weights):
    ...
    optimizer_classifier = torch.optim.Adam(parameter_classifier, lr=args.lr_b, weight_decay=0.0005)
    optimizer_feature = torch.optim.Adam(parameter_feature, lr=args.lr_b, weight_decay=0.0005)
...
def DGMA(X, Y, count_num, args):
    ...
    for i in range(args.max_iter2):
        ...
        for batch_idx, data in enumerate(datasets):
            ...
            #列表存储每个源域的批次数据=====================================================这里改特征：切空间特征========================================================
            for domain_idx in range(num_domains - 1):
                # key = 'Sx' + str(domain_idx + 1)
                # if count_num[domain_idx] == 0.1:
                #    continue
                # if key not in data:
                #   continue
                tmp_x = data['Sx' + str(domain_idx + 1)].float().cuda()
